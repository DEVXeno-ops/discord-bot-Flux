...
if (!fs.existsSync(commandsDir)) {
  logger.error('Commands directory not found at ./commands');
  process.exit(1);
}
...
client.on('interactionCreate', async interaction => {
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor('#ff0000')
            .setTitle('❗ Slow Down!')
            .setDescription(`Please wait ${timeLeft.toFixed(1)} seconds before using **/${interaction.commandName}** again.`)
  ...
  try {
    ...
    logger.error(`Error in ${interaction.commandName} by ${interaction.user.tag}`, error);
