  ...
  async execute(interaction) {
    ...
    try {
      if (!interaction.member.permissions.has(PermissionFlagsBits.ManageChannels)) {
        logger.warn(`Unauthorized lock attempt by ${interaction.user.tag} for ${channel.name}`);
      ...
      if (!interaction.guild.members.me.permissionsIn(channel).has(PermissionFlagsBits.ManageChannels)) {
        logger.warn(`Bot lacks Manage Channels permission in ${channel.name} for lock by ${interaction.user.tag}`);
        return interaction.reply({
          embeds: [
            new EmbedBuilder()
              .setColor('#ff0000')
      ...
      if (!channel.isTextBased()) {
        logger.warn(`Lock attempted in non-text channel ${channel.name} by ${interaction.user.tag}`);
        return interaction.reply({
          embeds: [
            new EmbedBuilder()
              .setColor('#ff0000')
      ...
      logger.info(`Locked ${channel.name} by ${interaction.user.tag}, reason: ${reason}`);
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor('#0099ff')
      ...
      logger.error(`Error in lock for ${channel.name} by ${interaction.user.tag}`, error);
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor('#ff0000')
